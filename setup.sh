...
if [[ -f .env ]]; then
  ...
  if [[ "$FORCE" == false ]]; then
    warn ".env already exists. Using existing settings."
...
if [[ -z "${ADMIN_PASSWORD_HASH:-}" ]] || [[ "$FORCE" == true ]]; then
  ...
  if command -v node &>/dev/null && node -e "require('bcryptjs')" &>/dev/null 2>&1; then
    ...
    ADMIN_PASSWORD_HASH=$(docker run --rm node:20-alpine \
      sh -c "npm install -g bcryptjs 2>/dev/null; node -e \"const b=require('bcryptjs');process.stdout.write(b.hashSync('$ADMIN_PASSWORD',10))\"")
    success "Generated hash using Docker"
  fi

else
  info "Skipping configuration (using existing .env)"
